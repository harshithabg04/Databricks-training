...
**Query #1**
    ...
    --  LENGTH / CHAR_LENGTH
    SELECT full_name, LENGTH(full_name) AS len, CHAR_LENGTH(full_name) AS char_len
    FROM employees;

| full_name       | len | char_len |
| --------------- | --- | -------- |
...
**Query #2**
    ...
    --  UPPER / LOWER
    SELECT department, UPPER(department) AS dept_upper,
           city, LOWER(city) AS city_lower
    FROM employees;

| department       | dept_upper       | city        | city_lower  |
| ---------------- | ---------------- | ----------- | ----------- |
| Data Engineering | DATA ENGINEERING | Hyderabad   | hyderabad   |
| Analytics        | ANALYTICS        | Bangalore   | bangalore   |
| Data Science     | DATA SCIENCE     | Chennai     | chennai     |
| DEVOPS           | DEVOPS           | Pune        | pune        |
| data engineering | DATA ENGINEERING |  hyderabad  |  hyderabad  |
...
**Query #5**
    ...
    SELECT CONCAT_WS(' | ', emp_id, full_name, city) AS emp_info
    FROM employees;
...
**Query #6**
    ...
    SELECT email,
           SUBSTRING(email, 1, 7) AS substr1,
           SUBSTR(email, 1, 5) AS substr2,
           MID(email, 1, 6) AS mid_example
    FROM employees;

| email               | substr1 | substr2 | mid_example |
| ------------------- | ------- | ------- | ----------- |
| karthik.k@gmail.com | karthik | karth   | karthi      |
| veena_r@company.com | veena_r | veena   | veena_      |
| ravi.kumar@org.in   | ravi.ku | ravi.   | ravi.k      |
| anil@abc.com        | anil@ab | anil@   | anil@a      |
| suresh@xyz.com      | suresh@ | sures   | suresh      |

---
**Query #7**

    --  LEFT / RIGHT
    SELECT full_name, LEFT(full_name, 4) AS left4,
           city, RIGHT(city, 3) AS right3
    FROM employees;

| full_name       | left4 | city        | right3 |
| --------------- | ----- | ----------- | ------ |
| Karthik Kondpak | Kart  | Hyderabad   | bad    |
| Veena Reddy     | Veen  | Bangalore   | ore    |
| Ravi kumar      | Ravi  | Chennai     | nai    |
| Anil            | Anil  | Pune        | une    |
|  Suresh         |  Sur  |  hyderabad  | ad     |
...
**Query #10**
    ...
    -- 1 REPLACE
    SELECT remarks, REPLACE(remarks, ' ', '_') AS replaced_spaces
    FROM employees;

| remarks                 | replaced_spaces         |
| ----------------------- | ----------------------- |
|  Top performer          | _Top_performer_         |
| Excellent communication | Excellent_communication |
...
**Query #12**
    ...
    --  FIELD / FIND_IN_SET
    SELECT department, FIELD(department, 'Analytics','DEVOPS','Data Engineering') AS field_pos
    FROM employees;

| department       | field_pos |
| ---------------- | --------- |
| Data Engineering | 3         |
| Analytics        | 1         |
| Data Science     | 0         |
...
**Query #13**
    ...
    SELECT city, FIND_IN_SET(city, 'Hyderabad,Bangalore,Chennai,Pune') AS find_pos
    FROM employees;
...
**Query #14**
    ...
    --  LPAD / RPAD
    SELECT full_name,
           LPAD(full_name, 20, '*') AS left_padded,
           RPAD(full_name, 20, '-') AS right_padded
    FROM employees;
...
**Query #15**
    ...
    --  REPEAT
    SELECT full_name, REPEAT('*', 5) AS stars
    FROM employees;
...
**Query #16**
    ...
    --  SPACE
    SELECT full_name, CONCAT(full_name, SPACE(5), department) AS spaced_concat
    FROM employees;

| full_name       | spaced_concat                        |
| --------------- | ------------------------------------ |
